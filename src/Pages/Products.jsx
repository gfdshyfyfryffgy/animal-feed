...
function ProductCard({ title, icon, color, iconColor, items, prefix }) {
  ...
  return (
    <div
      ...
      className="min-w-[260px] max-w-[275px] bg-white rounded-2xl shadow-md border border-[#F6F1DE] p-6 mx-2 flex flex-col items-center transition hover:scale-[1.03] hover:shadow-xl group cursor-pointer"
    >
      <div className={`w-14 h-14 ${iconColor} rounded-full flex items-center justify-center text-3xl mb-3`}>
        {icon}
      </div>
      <div className={`font-bold text-lg mb-2 ${color} group-hover:text-[#8AB2A6]`}>{title}</div>
      <ul className="text-sm text-[#3E3F5B] font-medium space-y-1 w-full">
        {items.length > 0 ? (
          items.map((item) => (
            <li
              key={item.slug}
              ...
              className="hover:text-[#8AB2A6] transition cursor-pointer"
            >
              {item.name}
            </li>
          ))
